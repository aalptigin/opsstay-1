// src/lib/sheets.ts
import { getGoogleAccessToken } from "./sheetsAuth";
...
export async function appendRow(env: Env, sheetName: string, row: (string | number)[]) {
  ...
  const url = `${API}/${env.GOOGLE_SHEET_ID}/values/${encodeURIComponent(
    range
  )}:append?valueInputOption=USER_ENTERED`;
  return gsFetch(env, url, {
    method: "POST",
    body: JSON.stringify({ values: [row.map(String)] }),
  });
}
